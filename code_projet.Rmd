...
```{r}
# Examinons de plus près les distributions de chacune pour regroupement éventuel
distribution_variables_modalites_inf_15=lapply(base4[,variables_modalites_inf_15$Variable], function(x) round(prop.table(table(x))*100,2))

distribution_variables_modalites_inf_15
```
...
resume=data.frame(Variable=names(base_numeric),CV=cv_variables)
resume_tried=resume[order(-resume$CV),]
```
...
```
On rejette  au seuil de 2% l'hypothèse d'indépendance entre les 2 variables, elles ne sont donc pas indépendantes
...
data_explicatives <- data.frame(
  'MAT_TxPo'=base3$MAT_TxPo,
  'MAT_EterPo'=base3$MAT_EterPo
)

library(FactoMineR)
acm_result <- MCA(data_explicatives, graph = TRUE)
summary(acm_result)
```
...
fviz_mca_ind(acm_result, 
             repel = TRUE, # Evite les chevauchements de labels
...
```{r}
# Visualiser les modalités
fviz_mca_var(acm_result, 
             col.var = "red", # Couleur des catégories
             repel = TRUE)
```

```{r}
fviz_mca_biplot(acm_result, 
                repel = TRUE, # Evite les chevauchements
                col.ind = "blue", # Couleur des individus
...
k <- 2  # Nombre de groupes souhaités
groupes <- cutree(cah, k = k)

# Ajouter les groupes aux données
data_explicatives$Cluster <- as.factor(groupes)
##Visualisation
library(ggplot2)
ggplot(data_explicatives, aes(x = Cluster, fill = MAT_TxPo)) +
  geom_bar(position = "fill") +
  labs(
...
```{r}
library(ggplot2)
ggplot(data_explicatives, aes(x = Cluster, fill = MAT_EterPo)) +
  geom_bar(position = "fill") +
  labs(
    title = "Répartition de MAT_TxPo par Cluster",
    x = "Cluster",
